...
/**
 * Esta clase se encarga de realizar operaciones sobre la tabla reservas de la
 * base de datos. Tales como guardar los datos de la reserva, indicar si una
 * mesa está disponible para una fecha y hora determinada, obtener un listado de
 * mesas, obtener el historial de reservas de un cliente, obtener todas las
 * reservas del dia de hoy y obtener la mesa por el numero de mesa. Se encarga
 * de conectar a la base de datos y manejar las consultas necesarias para
...
public class ReservaDAO {
    ...
    /**
     * Este método se encarga de guardar los datos de la reserva en la base de
     * datos, en su correspondiente tabla.
     *
     * @param reserva es un objeto de tipo reserva con todos los datos
     * necesarios para hacer una reserva.
    ...
    public boolean realizarReserva(Reserva reserva) {
        ...
        try {
            ...
            ps.setString(1,
                    reserva.getFecha().format(DateTimeFormatter.ofPattern(
                            "dd/MM/yyyy")));
            ps.setString(2,
                    reserva.getHora().format(DateTimeFormatter.ofPattern(
                            "HH:mm:ss")));
            ps.setString(3,
    ...
    /**
     * Este método te indica si la mesa se encuentra disponible en una fecha y
     * hora determinada
     *
     * @param numMesa indica el número de la mesa que se revisara si esta
     * disponible
     * @param fecha indica la fecha en la que se revisara si esta disponible la
     * mesa
     * @param hora indica la hora que se revisara para saber si la mesa está
     * disponible
     * @return true en caso de que la mesa este disponible, en caso contrario
     * false
     */
    public boolean mesaDisponible(int numMesa,
            LocalDate fecha,
            LocalTime hora) {
                ...
                while (rs.next()) {
                    if (rs.getInt("mesa") == numMesa) {
                        ...
                        if ((rs.getInt("mesa") == 0) 
                            ...
                            LocalTime auxHoraF = LocalTime.parse(rs.getString(
                                    "hora_fin"),
                                    DateTimeFormatter.ofPattern("HH:mm:ss"));
        ...
        try {
            ...
            while (rs.next()) {
                ...
                if (rs.getString("hora_fin") != null) {
                    reserva.setTiempoFinalizacion(LocalTime.parse(rs.getString(
                            "hora_fin"),
                            DateTimeFormatter.ofPattern("HH:mm:ss")));
                } else {
                ...
                if (rs.getString("hora_inicio") != null) {
                    reserva.setTiempoOcupacion(LocalTime.parse(rs.getString(
                            "hora_inicio"),
                            DateTimeFormatter.ofPattern("HH:mm:ss")));
                } else {
                ...
                reserva.setMesa(new Mesa(mesaNum,
                        mesaCap,
                        mesaUbi,
                        reserva));
                ...
                reserva.setTarjeta(new Tarjeta(tNombre,
                        tEmi,
                        tNum,
                        tCodSeguridad));
    ...
    public ArrayList<Reserva> obtenerReservasDeHoy() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                vector = cliente.mostrarDatos(rs.getInt("id_cliente")); 
    ...
    public Mesa obtenerMesaPorNumero(int num) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Ubicacion ubicacion = Ubicacion.valueOf(rs.
                        getString("ubicacion"));
                Capacidad capacidad = Capacidad.valueOf(rs.
                        getString("capacidad"));
     ...
     */
    public boolean cambiarAsistencia(int numMesa,
            LocalTime hora,
            boolean asistencia) {
    ...
    /**
     * Método que guarda el tiempo de ocupación y el de finalización en caso de
    ...
    public ArrayList<Reserva> obtenerReservasPasadas() {
        ...
        String sql = "SELECT * FROM reserva"
                + " WHERE asistencia = 0 AND multa = 0";
        try {
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            ...
            while (rs.next()) {
                if (LocalDate.parse(rs.getString("fecha"),
                        formatoFecha).isBefore(hoy)) {

                    Reserva reserva = new Reserva();
                    reserva.setId(rs.getInt("id"));
                    String fechaString = rs.getString("fecha");
                    LocalDate fechaReserva = LocalDate.parse(fechaString,
                            formatoFecha);
                    reserva.setFecha(fechaReserva);
                    reserva.setComentario(rs.getString("comentario"));
                    reserva.setHora(LocalTime.parse(rs.getString("hora")));
                    if (rs.getInt("asistencia") == 0 
                            && (rs.getInt("multa")) == 0) {
    ...
    /**
     * Cuenta todas las asistencias en la base de datos de un cliente pasado por
     * parámetro.
     *
